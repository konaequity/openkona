...
### Pipeline bugs and gaps (KARL paper vs KONASH code)
- [x] **"Parallel" thinking is sequential.** Fixed — both `ParallelThinkingEngine` and `ParallelThinkingStrategy` now use `ThreadPoolExecutor` for concurrent rollouts.
- [x] **Rollout generator bypasses the harness.** Fixed — `generate_single()` now uses `Environment.run_episode()` with the same `BaseAgent`, `CompressionPlugin`, and `StepBudgetPlugin` as inference. Single code path for training and serving.
- [x] **Solver rollouts use flat summaries, not multi-turn conversation.** Fixed — `generate_single()` now uses `Environment.run_episode()` which maintains proper multi-turn conversation (assistant → tool_calls → tool results → assistant). The old `_reason_with_llm()` flat-summary code was removed.
- [x] **Quality filter never receives rollout attempts.** Fixed — `run_stage_two()` now builds `rollout_attempts` from `filtered_groups` (answer, score, passed per rollout) and passes them to the quality filter, enabling the paper's structured LLM judge prompts (Figures 35-36).
- [ ] **Score binarization is dead code.** `PassRateFilter` has `binarize_scores()` but it's never called. TREC-Biogen pass rates are computed from raw continuous scores instead of binarized at increasing thresholds per iteration (0.6, 0.7, 0.75, 0.9).
- [ ] **Dedup silently degrades to trigram hashing.** If `sentence-transformers` isn't installed (common on machines without GPU), `DeduplicationAgent` falls back to character trigram pseudo-embeddings with no warning. Near-duplicates slip through and contaminate training data.
- [ ] **RL-trainable compression plugin isn't wired up.** `RLTrainableCompressionPlugin` exists but only `CompressionPlugin` (heuristic summarization) is imported anywhere. The RL-trained compression that KARL shows is critical (0.389 → 0.570) is never used by default.
- [ ] **VGS aggregation uses wrong scores.** `ValueGuidedSearchEngine` passes BFS beam scores (partial trajectory) to the aggregator as weights. KARL uses σ(V(x, y_complete)) — the value model's score on the finished rollout — which is a different signal.
- [ ] **No convergence monitoring.** No evaluation between training iterations, no pass@k tracking, no way to tell if training is improving the model without manually running eval.
- [ ] **Token masking uses rough character-count heuristic.** `_estimate_tool_ranges_from_steps()` estimates token ranges proportionally from character counts. Characters-to-tokens ratios vary widely, so the OAPL loss mask could be misaligned, corrupting the policy gradient.
- [ ] **Nothing enforces same model for synthesis and solving.** KARL requires this for iterative bootstrapping. KONASH builds `QuestionAnswerSynthesizer` and `RolloutGenerator` independently — they could silently use different models.
- [ ] **No per-task chunking or embedding model.** KARL uses different chunking strategies (512 tokens, page-level, sentence-level, 2048 tokens) and different embedding models per dataset. KONASH uses uniform 512-word chunks and a single embedding approach for everything.
- [ ] **Aggregation rollout doesn't mention tool access.** KARL's aggregator can make search calls during aggregation (improves answer 23.7% of the time on PMBench). KONASH's aggregation prompt doesn't instruct the model that tools are available.
